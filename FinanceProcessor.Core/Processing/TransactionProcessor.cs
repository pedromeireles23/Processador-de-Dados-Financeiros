...
namespace FinanceProcessor.Core.Processing
{
  public class TransactionProcessor
  {
    ...
    decimal GetMultiplier(string tipo) => tipo switch
    {
     "Crédito" => 1m,
     "Débito" => -1m,
     _ => 0m
    ...
    public Dictionary<string,decimal> GetMonthlySummary(IEnumerable<Transaction> transactions)
    {
      ...
      return transactions.GroupBy(t=> t.Data.ToString("yyyy-MM")).ToDictionary(grupo => grupo.Key,grupo => grupo.Sum(t => SaldoCalculado(t)));
    }
    ...
    public Dictionary<string,decimal>GetTopExpenses(IEnumerable<Transaction> transactions)
    {
      ...
      return transactions.Where(t=> t.Tipo == "Débito").GroupBy(t=>t.Categoria).Select(grupo=> new
      {
        Categoria = grupo.Key,
        Total = grupo.Sum(t=> t.Valor)
      }).OrderByDescending(x=> x.Total).Take(10).ToDictionary(x=> x.Categoria, x=> x.Total);
    }
    ...
    public Dictionary<DateOnly, decimal> GetDailyBalance (IEnumerable<Transaction> transactions)
    {
      ...
      var saldoIsoladoPorDia = transactions.GroupBy(t=> t.Data).Select(grupo => new
      {
        Data = grupo.Key,
